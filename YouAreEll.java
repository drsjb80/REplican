...
public class YouAreEll
{
    ...
    private void dealWithStopOns (int code)
    {
        ...
        for (int i = 0; i < stopon.length; i++)
        {
            if (code == stopon[i])
            {
                logger.warning ("Stopping on return code: " + code);
                System.exit (0);
            }
        }
    }
        ...
        switch (code)
        {
            ...
            default:
            {
                try
                {
                    ...
                    logger.warning ("For: " + url + " server returned: " +
                        code + " " + message);
                }
                catch (IOException e)
                {
        ...
        try
        {
            ...
            if (args.Header != null)
            {
                for (int i = 0; i < args.Header.length; i++)
                {
                    ...
                    {
                        logger.finest ("Couldn't decipher " + args.Header[i]);
                    }
                }
            }
    ...
    private InputStream getURLInputStream ()
    {
        ...
        logger.finest (uc.toString());
        logger.finest (uc.getHeaderFields().toString());
        ...
        if (! args.IgnoreCookies)
        {
            ...
            if (l != null)
            {
                for (ListIterator i = l.listIterator(); i.hasNext(); )
                {
                    ...
                    logger.finest ("Adding cookie: " + cookie);
                    try
                    {
